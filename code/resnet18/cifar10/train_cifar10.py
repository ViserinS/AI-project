...
class Trainer:
    def __init__(self, args):
        ...
        # 创建保存目录
        self.checkpoint_dir = 'cifar10/checkpoints_only_colorjitter'
        os.makedirs(self.checkpoint_dir, exist_ok=True)
        
        # 设置tensorboard
        run_id = datetime.datetime.now().strftime('%Y%m%d-%H%M%S')
        self.writer = SummaryWriter(f'runs/cifar10_only_colorjitter{run_id}')
    ...
    def _setup_model_for_fashin_mnist(self):
        """Setup model with single channel input"""
        self.model = resnet18(pretrained=False)
...
def main():
    ...
    # trainer._load_checkpoint('checkpoints_GaussianNoise/checkpoint_epoch_59.pth')
    trainer.train()
